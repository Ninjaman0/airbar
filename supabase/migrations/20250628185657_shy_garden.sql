...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'external_money' AND rowsecurity = true
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'supplement_debt_transactions' AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'external_money' AND policyname = 'Allow all operations on external_money'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'supplement_debt_transactions' AND policyname = 'Allow all operations on supplement_debt_transactions'
  ) THEN
